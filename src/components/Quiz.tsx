...
export const Quiz = ({ onComplete }: QuizProps) => {
  ...
  // Detectar tentativa de saída durante o quiz (desktop e mobile)
  useEffect(() => {
    ...
    const handleScroll = () => {
      // Mobile: detectar scroll para cima rápido
      if (!showStart && currentQuestion < questions.length && !hasShownExitModal) {
        ...
        if (currentScrollY < lastScrollY) {
          ...
          // Se rolou para cima rapidamente (3 vezes consecutivas)
          if (scrollUpCount >= 3) {
            setShowExitModal(true);
            setHasShownExitModal(true);
          }
        } else {
          // Reset contador se rolar para baixo
          scrollUpCount = 0;
        }
    ...
    const handlePopState = (e: PopStateEvent) => {
      // Mobile: detectar uso do botão voltar
      if (!showStart && currentQuestion < questions.length && !hasShownExitModal) {
        e.preventDefault();
        setShowExitModal(true);
        setHasShownExitModal(true);
    ...
    const handleBeforeUnload = (e: BeforeUnloadEvent) => {
      // Desktop: detectar fechamento da aba
      if (!showStart && currentQuestion < questions.length && !hasShownExitModal) {
        e.preventDefault();
        e.returnValue = '';
        setShowExitModal(true);
  ...
  const questions = [
    ...
    {
      id: 2,
      question: "Nos últimos 6 meses, você sentiu queda no seu desempenho sexual?",
      options: [
    ...
    {
      id: 4,
      question: "Sua disposição para sexo é igual ou maior do que há 5 anos?",
      options: [
  ...
  return (
    <>
      ...
      <ExitIntentModal 
        ...
        continueButtonText="Continuar o teste"
      />
    </>
  );
};
